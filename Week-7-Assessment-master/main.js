...
function MultiByNum1OrNum2(x,Fnum,Snum) {

        var newArr=x.map(function(x){
            if(x%Fnum==1){
                return x * Snum
            }
            else {
...
function longerAndYounger(arr,element1,element2) {

 var newArray = arr.filter(object=>{
 if (object.name.length > element1 &&  object.age < element2){
     return newArray=element1;
 }
 }); 
 return newArray; 
}
